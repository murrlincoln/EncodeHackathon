...
const AccountInfo = ({ address, session }: IAccountInfo) => {
	return (
		<div className="AccountInfo">
			<h2 className="AccountInfo-h2">Account Info</h2>
			{session?.ens &&
				(session?.ens.domain ||
				...
				<div>
					<b className="AccountInfo-label">ENS</b>
					<br />
					<div className="AccountInfo-container">
						...
						{session.ens.domain || session.ens.ensName ? (
							<code id="ensDomain" className="AccountInfo-value">
								{session.ens.domain || session.ens.ensName}
							</code>
						) : null}
					</div>
			...
			{session?.lens ? (
				typeof session.lens !== 'string' ? (
					session.lens.items.length > 0 ? (
						<div>
							<b className="AccountInfo-label">
								Lens
								{session.lens.pageInfo.totalCount > 10 ? (
									<small>&nbsp;(listing first 10)</small>
								) : null}
							</b>
							<br />
							{session.lens.items.map(
					...
					<div>
						<b className="AccountInfo-label">Lens</b>
						<div className="AccountInfo-container">
							<code className="AccountInfo-value">{session.lens}</code>
						</div>
					</div>
				)
			) : null}
			<p>
				<b className="AccountInfo-label">Address</b>
				<br />
				<code id="userAddress" className="AccountInfo-value">
					{address?.toLowerCase()}
				</code>
			</p>
		</div>
